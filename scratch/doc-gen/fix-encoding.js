...
Object.entries(diagrams).forEach(([filename, content]) => {
    ...
    // Delete existing file if it exists to clear any encoding metadata issues
    if (fs.existsSync(fullPath)) {
        fs.unlinkSync(fullPath);
    }
    // Add UTF-8 BOM to force Windows Notepad to read it as UTF-8
    const bom = '\\uFEFF';
    fs.writeFileSync(fullPath, bom + content, { encoding: 'utf8' });
});

console.log("All files deleted and recreated with correct UTF-8 BOM encoding!");
